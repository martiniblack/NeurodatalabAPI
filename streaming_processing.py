...
class WebCamEmotionRecognitionStreamProcessing(IWebCamStreamProcessing):
    def visualize_result(self, image, res=None):
        # Check res has normal format
        if res is not None and len(res) > 0:
            # Visualize each faces in result
            for emotion in res[0]:
                # Get face coordinates
                x, y, w, h = list(map(int, [emotion[k] for k in ('x', 'y', 'w', 'h')]))
                emotions = emotion['emotions']
                emotion_pred = np.array(emotions[0][1])

                # Reading an emoji in default mode
                path = 'Emojis/{}.png'.format(str(emotion_pred))
